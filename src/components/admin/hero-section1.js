...
export default function HeroSection1({ formData, setFormData }) {
  ...
  if (!formData.heroSection1) {
    formData.heroSection1 = { heading: '', description: '', sliderImages: [] };
  }
  ...
  const handleImageUpload = (index, e) => {
    ...
    try {
      ...
      setFormData(prev => {
        const newSliderImages = [...(prev.heroSection1?.sliderImages || [])];
  ...
  const handleAddImage = () => {
    setFormData(prev => ({
      ...
      heroSection1: {
        ...prev.heroSection1,
        sliderImages: [...(prev.heroSection1?.sliderImages || []), { src: '', alt: '' }]
      }
    }));
  };
  ...
  const handleRemoveImage = (index) => {
    setFormData(prev => ({
      ...
      heroSection1: {
        ...prev.heroSection1,
        sliderImages: prev.heroSection1.sliderImages.filter((_, idx) => idx !== index)
      }
    }));
  };
